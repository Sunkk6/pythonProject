...
def resnet_Model(n_dim=10):
    ...
    """--------第二层----------"""
    out_dim = 128
    identity_2 = tf.keras.layers.Conv2D(filters=out_dim, kernel_size=3, padding="SAME", activation=tf.nn.relu)(
        identity_1)
    ...
    """--------第三层----------"""
    out_dim = 256
    identity_3 = tf.keras.layers.Conv2D(filters=out_dim, kernel_size=3, padding="SAME", activation=tf.nn.relu)(
        identity_2)
    ...
    """--------第四层----------"""
    out_dim = 512
    identity_4 = tf.keras.layers.Conv2D(filters=out_dim, kernel_size=3, padding="SAME", activation=tf.nn.relu)(
        identity_3)
